...
namespace TestConsole
{
    internal class Task_B
    {
        static void MainB(string[] args)
        {
            ...
            // Key - количество палуб, Value - количество кораблей
            var shipIdeal = new Dictionary<int, int> { { 1, 4 }, { 2, 3 }, { 3, 2 }, { 4, 1 } };

            for (var i = 0; i < testCaseCount; i++)
            {
                var ships = new Dictionary<int, int> { { 1, 0 }, { 2, 0 }, { 3, 0 }, { 4, 0 } };
                var hasError = Console.ReadLine().Split(' ')
                    .GroupBy(s => int.Parse(s))
                    .Any(s => !shipIdeal.ContainsKey(s.Key) || shipIdeal[s.Key] != s.Count());
                Console.WriteLine(hasError ? "NO" : "YES");
            }
        }
    }
}
