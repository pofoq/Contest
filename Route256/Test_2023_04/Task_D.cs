        ...
        public static void MainD(string[] args)
        {
            ...
            for (int i = 0; i < testCaseCount; i++)
            {
                ...
                for (int j = 0; j < length;)
                {
                    var start = arr[j++];
                    var count = 0;
                    if (j < length && start > arr[j])
                    {
                        while (j < length && (start - 1 + count) == arr[j])
                        {
                            count--;
                            j++;
                        }
                    }
                    else if (j < length && start < arr[j])
                    {
                        while (j < length && (start + 1 + count) == arr[j])
                        {
                            count++;
...
Формально, будем представлять результат компрессии как новую последовательность чётной длины вида b1,c1,b2,c2,…,bk,ck
, где пара идущих подряд членов bi,ci
 означает, что:

если ci≥0
, что в искомой последовательности был возрастающий отрезок вида bi,bi+1,bi+2,…,bi+ci
;
если ci<0
, что в искомой последовательности был убывающий отрезок вида bi,bi−1,bi−2,…,bi−|ci|
.
...
. Другой возможный результат сжатия: [3,−4,0,0,6,0,6,1]
.
