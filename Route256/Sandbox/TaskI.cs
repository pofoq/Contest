...
namespace Route256.Sandbox
{
    internal class TaskI : ITask
    {
        public void Run()
        {
            ...
            var cpus = Console.ReadLine()!.Split(' ').Select(s => long.Parse(s)).Select(n => (new Cpu(n), n)).ToArray();
            var free = new PriorityQueue<Cpu, long>(cpus);
            var busy = new PriorityQueue<Cpu, long>();
            ...
            for (int i = 0; i < taskCount; i++)
            {
                ...
                while (busy.TryDequeue(out cpu, out energy))
                {
                    if (cpu.IsFree(start))
                    {
                        cpu.BusyUntil = 0;
                        free.Enqueue(cpu, cpu.EnergyUse);
                    }
                    else 
                    { 
                        busy.Enqueue(cpu, cpu.BusyUntil);
                        break;
                    }
                }
                
                if (free.TryDequeue(out cpu, out energy))
                {
                    cpu.BusyUntil = start + duration - 1;
                    busy.Enqueue(cpu, cpu.BusyUntil);
                    result += duration * energy;
                }
            }
    ...
    public class Cpu : IComparable<Cpu>, IComparer<Cpu>
    {
        public long EnergyUse { get; }
        public int BusyUntil { get; set; }
        public Cpu(long energyUse)
        {
            EnergyUse = energyUse;
        }
        public bool IsFree(int time)
        {
            return BusyUntil < time;
        }

        public int CompareTo(Cpu? other)
        {
            return EnergyUse.CompareTo(other?.EnergyUse);
        }

        public int Compare(Cpu? x, Cpu? y)
        {
            return x!.EnergyUse.CompareTo(y!.EnergyUse);
        }
    }
}
/*
