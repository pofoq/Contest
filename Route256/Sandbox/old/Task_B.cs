...
namespace TestConsole.Codeforces.Sandbox
{
    public class Task_B
    {
        internal void Main(string[] args)
        {
            ...
            var promoProductCount = 3;

            for (int i = 0; i < testCaseCount; i++)
            {
                var testProductCount = int.Parse(Console.ReadLine());

                var arr = Console.ReadLine().Split(' ').Select(n => int.Parse(n)).ToArray();
                var sum = arr.GroupBy(n => n).Sum(g =>
                {
                    var price = g.Key;
                    var count = g.Count();
                    count -= count / promoProductCount;
                    return price * count;
                });
