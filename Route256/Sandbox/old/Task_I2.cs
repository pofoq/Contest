...
{
    // Частичное решение
    internal class Task_I2
    {
        static long Time = 0;
        public void Main(string[] args)
        {
            var input1st = Console.ReadLine().Split(' ').Select(s => long.Parse(s)).ToArray();
            var cpuCount = input1st[0];
            var taskCount = input1st[1];

            var cpuList = new List<CPU>();
            var taskList = new List<Task>();

            var energyArr = Console.ReadLine().Split(' ').Select(s => long.Parse(s)).ToArray();

            // CPU Energy
            for (int i = 0; i < cpuCount; i++)
            {
                cpuList.Add(new CPU(i + 1, energyArr[i]));
            }
            cpuList = cpuList.OrderBy(c => c.Energy).ToList();
            // Tasks
            for (long i = 0; i < taskCount; i++)
            {
                var taskArr = Console.ReadLine().Split(' ').Select(s => long.Parse(s)).ToArray();
                taskList.Add(new Task(taskArr[0], taskArr[1]));
            }
            ...
            {
                Time = task.Income;
                var cpu = cpuList.FirstOrDefault(c => c.IsFree);
                energySum += cpu?.Start(task) ?? 0;
            }
        ...
        class CPU
        {
            ...
            public bool IsFree => _busyUntil <= Time;

            private long _busyUntil;

            public CPU(int id, long energy)
            {
                Id = id;
                Energy = energy;
            }
            ...
            // EnegrySum
            public long Start(Task task)
            {
                _busyUntil = task.Income + task.Duration;
                return Energy * task.Duration;
            }
        }
        ...
        class Task
        {
            ...
            public Task(long income, long duration)
            {
                Income = income;
                Duration = duration;
            }
