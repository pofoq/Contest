...
namespace TestConsole.Codeforces.Sandbox
{
    public class Task_C
    {
        internal void Main(string[] args)
        {
            ...
            {
                var staffCount = int.Parse(Console.ReadLine());
                var staff = Console.ReadLine().Split(' ')
                    .Select(n => int.Parse(n))
                    .Select((value, index) => new { Key = index + 1, Value = value })
                    .ToDictionary(p => p.Key, p => p.Value);

                int first = 0, second = 0, firstSkill, secondSkill, skillDif;

                while (staff.Count() > 0)
                {
                    first = staff.First().Key;
                    staff.Remove(first, out firstSkill);
                    secondSkill = -1;
                    skillDif = 0;
                    ...
                    foreach (var temp in staff)
                    {
                        if (secondSkill < 0)
                        {
                            skillDif = secondSkill = Math.Abs(firstSkill - temp.Value);
                            second = temp.Key;
                        }
                        else
                        {
                            if (skillDif > Math.Abs(firstSkill - temp.Value))
                            {
                                skillDif = Math.Abs(firstSkill - temp.Value);
                                second = temp.Key;
                            }
                        }
                    }
