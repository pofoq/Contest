        ...
        public void Run()
        {
            ...
            for (var i = 0; i < caseCount; i++)
            {
                ...
                var workers = Console.ReadLine()!
                    ...
                    .ToDictionary(pair => pair.index + 1, pair => int.Parse(pair.val));

                var list = new LinkedList<KeyValuePair<int, int>>(workers);
                ...
                while (list.Count > 1)
                {
                    ...
                    int dif = Math.Abs(first.Value.Value - current.Value.Value);
                    int nextDif;
                    while (dif != 0 && next is not null)
                    {
                        nextDif = Math.Abs(first.Value.Value - next.Value.Value);
                        if (dif > nextDif)
                        {
                            dif = nextDif;
                            current = next;
                    ...
                    Console.WriteLine($"{first.Value.Key} {current.Value.Key}");
                    list.RemoveFirst();
                    list.Remove(current);
                }
