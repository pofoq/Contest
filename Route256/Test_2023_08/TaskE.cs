...
namespace Route256.Test_2023_08
{
    internal class TaskE : ITask
    {
        public void Run()
        {
            ...
            {
                var reportCount = int.Parse(Console.ReadLine()!);
                var report = Console.ReadLine()!.Split(' ').Select(s => int.Parse(s)).ToArray();
                Console.WriteLine(Check(report, reportCount) ? "YES" : "NO");
            }

            bool Check(IList<int> report, int reportCount)
            {
                var current = report[0];
                var unique = new HashSet<int>() { current };
                for (var j = 1; j < reportCount; j++)
                {
                    if (current != report[j])
                    {
                        if (unique.Contains(report[j]))
                        {
                            return false;
                        }
                        current = report[j];
