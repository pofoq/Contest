...
namespace TestConsole
{
    ...
    internal class Task_I
    {
        ...
        {
            var line = Console.ReadLine().Split(' ').Select(o => Convert.ToInt64(o)).ToArray();

            var cpuCount = line[0];
            var taskCount = line[1];

            var cpus = Console.ReadLine().Split(' ').Select(o => Convert.ToInt64(o)).OrderBy(o => o).ToList();
            var idls = new PriorityQueue<long, long>(cpus.Select(o => (o, o)));
            ulong total = 0;
            long currentTime = 0;
            var busy = new PriorityQueue<long, long>();
            for (int i = 0; i < taskCount; i++)
            {
                line = Console.ReadLine().Split(' ').Select(o => Convert.ToInt64(o)).ToArray();
                var time = line[0];
                var duration = line[1];
                var ended = time + duration;
                currentTime = time;
                {
                    while (busy.TryPeek(out var cpu, out var t))
                    {
                        if (t <= currentTime)
                        {
                            idls.Enqueue(cpu, cpu);
                            busy.Dequeue();
                        }
                        else
                        {
                ...
                {
                    if (idls.TryDequeue(out var cpu, out var _))
                    {
                        busy.Enqueue(cpu, ended);
                        total += (ulong)cpu * (ulong)duration;
                    }
                }
            }
        }
